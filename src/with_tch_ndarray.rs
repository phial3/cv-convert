...
impl<A, D> TryToCv<ndarray::Array<A, D>> for tch::Tensor
where
    D: ndarray::Dimension,
    A: tch::kind::Element,
    ...
    Vec<i64>: ToNdArrayShape<D, Error = Error>,
{
    ...
    fn try_to_cv(&self) -> Result<ndarray::Array<A, D>, Self::Error> {
        ...
        let array = ndarray::Array::from_shape_vec(array_shape, elems)?;
        Ok(array)
    }
}

impl<A, S, D> ToCv<tch::Tensor> for ndarray::ArrayBase<S, D>
where
